...
// ── URL-based page detection (mirrors content.js logic) ─────────
function detectPageFromUrl(url) {
  try {
    const section = new URL(url).searchParams.get('section') || ''
    if (section === 'personalDetails')   return 'personal_info'
    if (section === 'languageTest' || section === 'language') return 'language'
    if (section === 'educationHistory' || section === 'education') return 'education'
    if (section === 'workHistory' || section === 'employment') return 'work_history'
    if (section === 'spouseInfo' || section === 'spouse') return 'spouse'
    if (section === 'adaptability') return 'adaptability'
...
async function detectCurrentPage() {
  ...
  try {
    ...
    // First: detect from URL directly (reliable even if content script not injected)
    let page = detectPageFromUrl(url)

    // Second: try content script for more accurate detection
    if (page === 'unknown') {
      try {
        const result = await chrome.tabs.sendMessage(tab.id, { type: 'DETECT_PAGE' })
        page = result?.page || 'unknown'
      } catch {
        // Content script not injected — try to inject it now
        try {
          await chrome.scripting.executeScript({ target: { tabId: tab.id }, files: ['content.js'] })
          await new Promise(r => setTimeout(r, 500))
          const result2 = await chrome.tabs.sendMessage(tab.id, { type: 'DETECT_PAGE' })
          page = result2?.page || page
        } catch {}
      }
    }

    pageLabel.textContent = result?.label || PAGE_LABELS[page] || page
